...
本文件是翻译语体的校准锚点：每组样例是一段英文原文与一段中文译文，覆盖本仓库文档的主要文体。**译文的语体以这些样例为准**——文体样例的效力高于对语气的文字描述，但术语表、忠实性与结构规则仍然优先。翻译或评审时对照最接近的文体样例。本文件中英对照、自成双语，不参与配对（见 [README.md](README.md) 排除清单）。


## ① 架构叙述

> This document describes the architecture of the platform. The governing principle, from the microkernel design discussion: **everything is a plugin**. The core is deliberately tiny — a handful of abstract services plus one concrete loop plugin (`core-loop`) — and every product feature is a plugin against the extension API described here, without modifying the loop.

本文介绍平台整体架构。微内核设计讨论中确立了核心设计准则：**一切皆插件**。内核刻意做得极精简，仅包含少量抽象服务，外加一个实体循环插件 `core-loop`。所有产品功能均基于本文定义的扩展接口开发为独立插件，无需改动主循环逻辑。

> Dependency rule: extension plugins depend on interfaces, never on `core-loop` (the loop is swappable); the sanctioned exception is the composition bundle `spine-demo`, whose job is assembling the concrete spine.

依赖约束规范：各类扩展插件仅依赖抽象接口，严禁直接依赖 `core-loop`（该主循环支持替换实现）；唯一允许的特例是组合包 `spine-demo`，它的职责是组装整套实体主干。
...
- 用中文工程惯用语替换直译：false positive/negative→误报／漏检、postmortem→事故复盘、smoke test→冒烟测试。
...
- 代码体标识符（事件名 `agent/status`、状态值 `running`、包名 `bash-runner` 等）在译文中保留 code span 原文，不得口语化改写；Pass 2 必须逐句核验。
