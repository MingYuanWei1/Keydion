...
本文件是自动翻译流水线的 prompt 模板；从 `# Translation Prompt` 开始的正文会逐字进入模型请求，因此本文件不参与双语配对（见 [README.md](README.md) 排除清单）。模板正文与内嵌 few-shot 正误例经人工质量评审定稿，是流水线行为的基线。渲染时把 [terminology.md](terminology.md) 整表填入 `{{terminology}}`；除此之外不注入任何其他仓库文件（translation-rules.md 约束人和 agent 的翻译工作，不注入本模板）。[style-samples.md](style-samples.md) 定义文体，模板中的 Examples 只用于说明典型问题，两者冲突时以文体样例为准。修改本文件会改变翻译行为，需正常经过 PR（Pull Request）评审。
...
流水线使用**整篇文档**的中英对照作为 few-shot，不是模板内嵌的句子级正误例。few-shot 语料取本仓库经过人工评审的双语配对，以仓库当前版本为准、随仓库更新；基线语料是本标准自身的两组配对：

- `docs/i18n/README.md` ↔ `docs/i18n/README.zh.md`
- `docs/i18n/translation-rules.md` ↔ `docs/i18n/translation-rules.zh.md`

仓库可按文体覆盖需要补充其他已评审配对。注入方式：在系统消息（本模板）之后、待译文档之前，每组作为一轮示例对话——user 消息为源文档全文，assistant 消息为定稿译文全文（裸文本，不带三段 XML 包装；只有真实请求要求三段输出）。上下文不足时按列表顺序从后往前删减组数。这些配对也是评审校准锚点（见 [style-samples.md](style-samples.md)），改动任何一组即改变流水线行为。
...
- Source: `The committed agent workflow lives in tools/translate-docs`
- Bad: `进仓的 agent 工作流见 tools/translate-docs`
- Good: `仓库内置的 agent 工作流见 tools/translate-docs`
...
- Source code block contains: `# full-screen TUI coding agent (needs API_KEY)`
- Bad: `# 全屏 TUI coding agent（需要 API_KEY）`
- Good: `# full-screen TUI coding agent (needs API_KEY)` (keep exactly as-is, byte-for-byte)
