...
本文规定：如何翻译文档配对的中英文两侧。两种语言同权（见 [README.md](README.md)）：每次变更可以用任一语言撰写，被编辑的一侧即为本次更新的源；本文的规则约束如何产出或更新对侧文件。这些规则对人类和 agent（智能体）同等生效。日常工作中，agent 会在术语指导下直接一次完成有改动内容的翻译。规则级别沿用 RFC 2119 的用法：**必须（MUST）**／**禁止（MUST NOT）** 人工评审；**应当（SHOULD）** 偏离时要说明理由；**可以（MAY）** 自行裁量。
...
- 优先采用目标语言中通行的工程表达，避免生硬直译（如 false positive/negative→误报／漏检）；隐喻应自然改写，名词链则按目标语言的习惯拆开。
...
- 围栏代码块：**逐字节一致，包括注释**。配对签名比对信息字符串与内容；
...
- 专有名词保持规范大小写：GitHub、TypeScript、PyTorch。除非引用代码，否则绝不写 `github`／`Github`。
