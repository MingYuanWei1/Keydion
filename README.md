...
- **单账号互斥登录**：同一账号仅允许一个活跃会话；若未正常退出，10 分钟无操作后自动释放，可通过环境变量调节时长。
- **论文元数据管理**：上传时记录标题、作者、关键词、单位、发表时间；在检索页面以表格形式展示，并可下载原始 PDF。
- **全文关键词检索**：支持在上传的 PDF 中按标题或正文内容匹配关键词。
- **PDF 上传与删除**：贡献者可上传，管理员可删除；系统阻止重名覆盖，并在删除前需二次确认。
- **现代化界面与双语支持**：基于 Bootstrap 5 定制设计，导航栏可即时切换中 / 英文。
...
3. 重启应用或刷新页面，即可看到语言更新。导航栏右侧可即时切换中文 / 英文。
...
- **分页检索与在线预览**：检索页支持分页跳转，点击标题将在新页面内嵌浏览 PDF，并提供下载按钮。
