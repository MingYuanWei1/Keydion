...
- **Dual Authentication**: Local PBKDF2-hashed passwords and Microsoft Graph OAuth for login and profile synchronization, with role-based access (Reader / Contributor / Curator).
...
- **Tesseract OCR** (optional) — enables local text extraction from *scanned* PDFs (chat attachments, the abstract/keyword generator, and the papers index). Install the engine plus the Chinese language data:
  ...
  When an LLM Vision provider is configured, vision reading is preferred and local Tesseract extraction is the fallback.
...
`requirements.txt` contains the application's direct dependency constraints.
`requirements.lock` is generated by `pip-tools` and pins both direct and
transitive dependencies, including hashes, for reproducible installations. Do
not edit `requirements.lock` manually. Generate it with the Python version used
by the production container (currently Python 3.14).

Install the lockfile tooling once in the virtual environment:

```bash
.venv/bin/python -m pip install pip-tools
```

After changing `requirements.txt`, update every dependency allowed by its
constraints. `--reuse-hashes` avoids recalculating hashes for unchanged package
versions:
...
To update only one direct or transitive dependency, replace `--upgrade` with a
targeted upgrade, for example:
...
If `pip-compile` appears stuck, interrupt it with `Ctrl+C` and test access to
the package index separately:

```bash
.venv/bin/python -m pip index versions Flask --timeout 15 --retries 1
```

If that succeeds, retry compilation with bounded network retries while keeping
...
Verify the generated lockfile before committing it:
...
Model requests run through the Cloudflare Worker in `workers/llm/`. Follow
...
page shows Worker capabilities; Tavily web search remains on Keydion.

Papers uploaded before embeddings were configured are not automatically embedded.
Once the Worker embedding capability matches the stored index, index any missing papers:
...
2. Merge new strings into each catalog (keeps existing translations, adds new
   strings as empty `msgstr`, and marks changed source text fuzzy):
   ```bash
...
4. Compile the translations:
   ...
   ```
   Flask-Babel loads `.mo` files at startup, so restart the dev server after
   compiling for new translations to appear.

## Project Structure

- `app.py`: App factory core — `create_app()`, auth/account/admin routes, template filters, and legacy redirects. Importing `app` does not connect to MySQL.
- `models.py`: SQLAlchemy ORM models, plus empty-database bootstrap and schema verification.
- `routes/`: HTTP routes per domain; each module exposes `register_routes(app)`.
- `services/`: Domain logic (DB/storage helpers) per domain; never imports `app`.
- `templates/`: Jinja2 templates for the web interface (dashboard panels in `templates/_dashboard/`).
- `static/`: CSS, JavaScript, and image assets; Bootstrap and other vendor libraries in `static/vendor/`.
- `data/`: JSON configuration (paper/news/guide categories, EE and IA subjects) and runtime `pending_papers/`.
- `papers/`: Storage for uploaded PDF files.
- `translations/`: Flask-Babel `en`/`zh` translation catalogs.
- `migrations/`: Alembic migrations — every schema change goes through Alembic.
- `tests/`: Contract tests (`unittest` with AST parsing and Jinja2 rendering).
- `tools/`: Administrative utility scripts (user management, translations, embeddings, workers).
- `deploy/`: Production nginx config and systemd units.
- `docs/`: Deployment runbook, architecture decision records, and agent documentation.
